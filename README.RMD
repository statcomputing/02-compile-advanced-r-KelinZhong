...
```
A:
Update all the R and LaTex packages
```{R}
update.packages(ask = FALSE, checkBuilt = TRUE)
tinytex::tlmgr_update()
tinytex:::install_prebuilt()
```
Q12 Front "Inconsolata" is missing after rebuild "tinytex" package:
```{R}
! Package fontspec Error: The font "Inconsolata" cannot be found.
```
A:
I tried to install Inconsolata font with its otf files in control panel but the "tinytex" package still failed to recognize this front.



Q13 Fail to compile _main.tex:
```{R}
LaTeX failed to compile _main.tex. See https://yihui.org/tinytex/r/#debugging for debugging tips. See _main.log for more info.
```

A:
Because the "tinytex" failed to recognize the "Inconsolata" font, the _main.tex could not be compiled.
