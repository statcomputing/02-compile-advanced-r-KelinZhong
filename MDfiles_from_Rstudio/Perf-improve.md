...
#> 1 mean1(x)      177μs  177.5μs     5275.
#> 2 mean2(x)       87μs   87.3μs    10800.
```
...
#>   expression           min   median `itr/sec`
#>   <bch:expr>      <bch:tm> <bch:tm>     <dbl>
#> 1 mean(x)            3.6μs    4.1μs   208417.
#> 2 mean.default(x)    1.3μs    1.6μs   449197.
```
...
#>   expression              min   median `itr/sec`
#>   <bch:expr>         <bch:tm> <bch:tm>     <dbl>
#> 1 mean(x)               3.6μs    4.1μs   216090.
#> 2 mean.default(x)       1.3μs    1.7μs   458756.
#> 3 .Internal(mean(x))    200ns    300ns  2998950.
```
...
#>   expression              min   median `itr/sec`
#>   <bch:expr>         <bch:tm> <bch:tm>     <dbl>
#> 1 mean(x)              21.2μs   21.7μs    42689.
#> 2 mean.default(x)      18.5μs   19.8μs    48119.
#> 3 .Internal(mean(x))   17.3μs   17.5μs    53141.
```
...
#>   expression         min   median `itr/sec`
#>   <bch:expr>    <bch:tm> <bch:tm>     <dbl>
#> 1 as.data.frame    1.2ms    1.3ms      706.
#> 2 quick_df         7.3μs    8.5μs   100427.
```
...
#>   expression        min   median `itr/sec`
#>   <bch:expr>   <bch:tm> <bch:tm>     <dbl>
#> 1 lookup[x1]      400ns    600ns  1359989.
#> 2 lookup[x10]     1.2μs    1.3μs   667628.
#> 3 lookup[x100]    2.6μs    4.3μs   213874.
```
...
#>   expression      min   median `itr/sec`
#>   <bch:expr> <bch:tm> <bch:tm>     <dbl>
#> 1 loop10       33.1μs   36.9μs    24876.
#> 2 loop100       865μs  883.7μs     1080.
#> 3 vec10           6μs    6.6μs   138121.
#> 4 vec100       34.2μs     35μs    26266.
```
